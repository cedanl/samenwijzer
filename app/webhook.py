...
def _valideer_twilio_handtekening(request: Request, auth_token: str, params: dict) -> bool:
...
async def whatsapp_webhook(
    ...
    if not auth_token:
        log.error("TWILIO_AUTH_TOKEN niet ingesteld — verzoek geweigerd")
        raise HTTPException(status_code=403, detail="Webhookauthenticatie niet geconfigureerd")
    form_data = dict(await request.form())
    if not _valideer_twilio_handtekening(request, auth_token, form_data):
