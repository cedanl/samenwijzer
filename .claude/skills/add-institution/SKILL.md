...
Since the instelling-registry, all per-institution metadata lives in **one source**:
`src/validatie_samenwijzer/instellingen.py` → `INSTELLINGEN` (dataclass `Instelling`: naam,
display_naam, map_naam, web_domein, klas_prefix, crawlbaar, diff_velden). All consumers
(`ingest._INSTELLINGEN`/`_MAP_NAAM`, `scripts/seed_bulk.py:INSTELLINGEN`,
`app_fastapi/main.py:_INSTELLING_KEYS`, `chat._INSTELLING_DOMEINEN`,
`bron_updates._OER_CRAWLBAAR`) derive their mapping from it automatically.
`tests/test_instelling_lijsten_sync.py` guards that the derived mappings cover the registry.

**Registry (the one edit):**
1. `src/validatie_samenwijzer/instellingen.py` → append a new `Instelling(...)` **at the END of
   the tuple**. The registry order is the seed order (one shared `Random(2026)`); inserting
   mid-list reshuffles every existing student. Set `crawlbaar` (can the catalogue be fetched
   automatically?) and `diff_velden` (narrow to `("crebo", "cohort")` if the catalogue lacks a
   reliable leerweg — else every OER looks "new" on the freshness diff).

**Only if applicable — still manual:**
2. `src/validatie_samenwijzer/oer_catalogus.py` → `_CATALOGUS_BRONNEN` — only if the institution
   gets a freshness scraper (Phase 1a + adapter registration).
3. `.gitignore` (repo root) → add `oeren/<key>_oeren/` **if the institution is private/Box-only**.

**Parser branches (only if the filename/title format is novel):**
4. `src/validatie_samenwijzer/ingest.py` → `parseer_bestandsnaam` / `_extraheer_opleiding_uit_pdf`.
...
5. `scripts/build_oer_catalog.py` → `_INSTELLING_DISPLAY` (currently STALE — missing 4 of 9).
6. `scripts/generate_synthetisch_data.py` → `_OVER_TE_SLAAN_INSTELLINGEN` / `_AANTAL_INSTELLINGEN`.
